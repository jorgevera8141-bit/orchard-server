...
async function resolveElementsList(resolvers, entries) {
  ...
  for (const entry of entries) {
    ...
    result.push(await core.resolveElements(resolvers, entry, { cache: false }));
  }
  return result;
}
async function resolveTransformMap(resolvers, transform, plugins) {
  ...
  for (const [pattern, value] of Object.entries(transform)) {
    ...
    result[pattern] = await core.getTransformerFunction(resolvers, value, plugins);
  }
  return result;
}
class FlatCompat {
  ...
  async config(data) {
    ...
    const config = await core.Config.fromObject(resolvers, data);
